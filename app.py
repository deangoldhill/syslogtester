...
</style></head><body><main><header><h1>Syslog <span class="pill">COMMAND CENTER</span></h1><p class="muted">Live ingest, correlation and access control — all local and dependency-free.</p></header>
...
<section><h2>Administration</h2><form id="add-admin"><input name="username" autocomplete="username" placeholder="New admin username" required><input name="password" type="password" autocomplete="new-password" placeholder="Password (12+ characters)" required><button>Add administrator</button></form><p class="muted">Each administrator can enable TOTP MFA from their session. Passwords are stored as slow salted hashes; the bootstrap password is only used for first-run setup.</p><div id="admins"></div></section>
...
def main():
    ...
    with db() as con:
        if not con.execute("SELECT 1 FROM admins LIMIT 1").fetchone():
            raise SystemExit('No administrator exists. Set SYSLOG_ADMIN_PASSWORD (12+ characters) for first startup.')
