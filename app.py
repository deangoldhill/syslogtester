...
TAB_BAR = '''<nav class="tabs"><a class="tab active" href="/">Dashboard</a><a class="tab" href="/config">Configuration</a><form method="post" action="/logout"><button class="logout" type="submit">Log out</button></form></nav>'''
PAGE = PAGE.replace('<header><h1>Syslog <span class="pill">COMMAND CENTER</span></h1><p class="muted">Live ingest, correlation and access control — all local and dependency-free. <a href="/config">Configuration</a></p></header>', '<header><h1>Syslog <span class="pill">COMMAND CENTER</span></h1><p class="muted">Live ingest, correlation and access control — all local and dependency-free.</p>'+TAB_BAR+'</header>').replace('</style>', '.tabs{display:flex;gap:8px;align-items:center;margin:12px 0}.tab,.logout{display:inline-block;padding:8px 13px;border-radius:7px;background:#273449;color:#dbeafe;text-decoration:none}.tab.active{background:#2563eb;color:white}.tabs form{margin-left:auto}.logout{border:0;cursor:pointer}</style>', 1)
CONFIG_PAGE = CONFIG_PAGE.replace('<header><h1>Configuration <span class="pill">COMMAND CENTER</span></h1><p class="muted">Manage listeners, administrator accounts, and MFA. <a href="/">Back to dashboard</a></p></header>', '<header><h1>Configuration <span class="pill">COMMAND CENTER</span></h1><p class="muted">Manage listeners, administrator accounts, and MFA.</p>'+TAB_BAR.replace('class="tab active" href="/"', 'class="tab" href="/"').replace('class="tab" href="/config"', 'class="tab active" href="/config"')+'</header>').replace('</style>', '.tabs{display:flex;gap:8px;align-items:center;margin:12px 0}.tab,.logout{display:inline-block;padding:8px 13px;border-radius:7px;background:#273449;color:#dbeafe;text-decoration:none}.tab.active{background:#2563eb;color:white}.tabs form{margin-left:auto}.logout{border:0;cursor:pointer}</style>', 1)

CONFIG_PAGE = CONFIG_PAGE.replace('onclick="enrolMfa()"', 'onclick="enrolMfa(${v.id})"').replace('v.id===x.current.id?', 'true?').replace('</script>', '''async function enrolMfa(id){let x=await api('/api/admins/'+id+'/totp',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});let verify=prompt(`Add this account in your authenticator with this secret:\n\n${x.secret}\n\nThen enter the six-digit verification code.`);if(!verify)return;await api('/api/admins/'+id+'/totp/confirm',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({code:verify})});alert('MFA enabled.');admins()}</script>''', 1)
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path.path == '/logout':
            clear_session(self)
            self.send_response(303); self.send_header('Set-Cookie', f'{COOKIE_NAME}=; HttpOnly' + ('; Secure' if COOKIE_SECURE else '') + '; SameSite=Strict; Path=/; Max-Age=0'); self.send_header('Location', '/'); self.end_headers()
            return
        if not self.require_auth(): return
        if path.path == '/api/admins':
            try:
        ...
        target = re.fullmatch(r'/api/admins/(me|\d+)/totp', path.path)
        if target:
            try:
                length = int(self.headers.get('Content-Length', '0')); data = json_object(self.rfile.read(length)) if length else {}
                current = session_admin(self); admin_id = current['id'] if target.group(1) == 'me' else int(target.group(1))
                with db() as con: account = con.execute("SELECT * FROM admins WHERE id=?", (admin_id,)).fetchone()
                if not account: return json_response(self, {'error': 'Administrator not found'}, 404)
                if admin_id == current['id'] and account["totp_secret"] and not consume_totp(admin_id, account["totp_secret"], str(data.get('code', ''))):
                    return json_response(self, {'error': 'Enter your current MFA code before replacing MFA'}, 403)
                secret = start_totp_enrolment(admin_id)
                return json_response(self, {'secret': secret, 'otpauth_uri': f"otpauth://totp/Syslog%20Command%20Center:{quote(account['username'])}?secret={secret}&issuer=Syslog%20Command%20Center&algorithm=SHA1&digits=6&period=30"})
            except (ValueError, json.JSONDecodeError) as exc: return json_response(self, {'error': str(exc)}, 400)
        target = re.fullmatch(r'/api/admins/(me|\d+)/totp/confirm', path.path)
        if target:
            try:
                length = int(self.headers.get('Content-Length', '0')); data = json_object(self.rfile.read(length))
                current = session_admin(self); admin_id = current['id'] if target.group(1) == 'me' else int(target.group(1))
                if not confirm_totp_enrolment(admin_id, str(data.get('code', ''))): return json_response(self, {'error': 'Invalid verification code'}, 400)
                cookie = SimpleCookie(self.headers.get("Cookie")); value = cookie.get(COOKIE_NAME)
                revoke_other_sessions(admin_id, value.value if value and admin_id == current['id'] else None)
                if value and admin_id == current['id']:
                    with sessions_lock:
